...
RUN cd /tmp && \
    ...
    yum -y install git tar freeglut-devel gmake glibc glibc-devel libX11 libX11-devel mesa-libGL mesa-libGL-devel wget nasm && \
    ...
    wget http://ftp.gnu.org/gnu/automake/automake-1.15.tar.gz && tar -xvzf automake-1.15.tar.gz && cd automake-1.15 && ./configure --prefix=/usr/local && make && make install && cd .. && \
		wget https://cmake.org/files/v3.7/cmake-3.7.2.tar.gz && tar -xf cmake-3.7.2.tar.gz && cd cmake-3.7.2 && ./configure --prefix=/usr/local && gmake && gmake install && cd .. && \
    rm -Rf m4-1.4.18* libtool-2.4.6* autoconf-2.69* automake-1.15* cmake-3.7.2*
