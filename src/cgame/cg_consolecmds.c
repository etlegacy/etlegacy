...
static void CG_CrosshairSize_f()
{
	if (trap_Argc() < 2)
	{
		trap_SendConsoleCommand(va("%s size\n", EDITCOMPONENT_CROSSHAIR_STRING));
	}
	else
	{
		trap_SendConsoleCommand(va("%s size %s %s position center\n", EDITCOMPONENT_CROSSHAIR_STRING, CG_Argv(1), CG_Argv(1)));
	}
}
...
static void CG_CrosshairAlpha_f()
{
	if (trap_Argc() < 2)
	{
		trap_SendConsoleCommand(va("%s colorMain\n", EDITCOMPONENT_CROSSHAIR_STRING));
	}
	else
	{
		trap_SendConsoleCommand(va("%s colorMain %f %f %f %s\n", EDITCOMPONENT_CROSSHAIR_STRING,
		                           CG_GetActiveHUD()->crosshair.colorMain[0],
		                           CG_GetActiveHUD()->crosshair.colorMain[1],
		                           CG_GetActiveHUD()->crosshair.colorMain[2],
		                           CG_Argv(1)));
	}
}
...
static void CG_CrosshairColor_f()
{
	...
	{
		trap_SendConsoleCommand(va("%s colorMain %s\n", EDITCOMPONENT_CROSSHAIR_STRING, CG_Argv(1)));
	}
	else
	{
		float r, g, b;
		...
		trap_SendConsoleCommand(va("%s colorMain %f %f %f\n", EDITCOMPONENT_CROSSHAIR_STRING, r, g, b));
	}
}
...
static void CG_CrosshairAlphaAlt_f()
{
	if (trap_Argc() < 2)
	{
		trap_SendConsoleCommand(va("%s colorSecondary\n", EDITCOMPONENT_CROSSHAIR_STRING));
	}
	else
	{
		trap_SendConsoleCommand(va("%s colorSecondary %f %f %f %s\n", EDITCOMPONENT_CROSSHAIR_STRING,
		                           CG_GetActiveHUD()->crosshair.colorSecondary[0],
		                           CG_GetActiveHUD()->crosshair.colorSecondary[1],
		                           CG_GetActiveHUD()->crosshair.colorSecondary[2],
		                           CG_Argv(1)));
	}
}
...
static void CG_CrosshairColorAlt_f()
{
	...
	{
		trap_SendConsoleCommand(va("%s colorSecondary %s\n", EDITCOMPONENT_CROSSHAIR_STRING, CG_Argv(1)));
	}
	else
	{
		float r, g, b;
		...
		trap_SendConsoleCommand(va("%s colorSecondary %f %f %f\n", EDITCOMPONENT_CROSSHAIR_STRING, r, g, b));
	}
}

static void CG_CrosshairPulse_f()
{
	int style = CG_GetActiveHUD()->crosshair.style;

	if (trap_Argc() < 2)
	{
		trap_SendConsoleCommand(va("%s style\n", EDITCOMPONENT_CROSSHAIR_STRING));
		return;
	}
	...
	trap_SendConsoleCommand(va("%s style %d\n", EDITCOMPONENT_CROSSHAIR_STRING, style));
}
