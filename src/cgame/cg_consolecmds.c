...
static void CG_TeamVoiceChat_f(void)
{
	...
	if (cg.snap && (cg.snap->ps.pm_type != PM_INTERMISSION))
	{
		if (cgs.clientinfo[cg.clientNum].team == TEAM_FREE)
		{
			CG_Printf("%s", CG_TranslateString("Can't team voice chat as TEAM_FREE.\n")); // FIXME? find a way to print this on screen
			return;
		}
	}
...
/**
 * @brief CG_ShoutcastAddScore
 * @param[in] team
 * @param[in] value
 */
static void CG_ShoutcastAddScore(team_t team, int value)
{
	if (team == TEAM_AXIS)
	{
		trap_Cvar_Set("cg_shoutcastTeamScoreRed", va("%i", cg_shoutcastTeamScoreRed.integer + value));
	}
	else
	{
		trap_Cvar_Set("cg_shoutcastTeamScoreBlue", va("%i", cg_shoutcastTeamScoreBlue.integer + value));
	}
}

/**
 * @brief CG_ShoutcasterPlusRedScore
 */
static void CG_ShoutcasterPlusRedScore(void)
{
	CG_ShoutcastAddScore(TEAM_AXIS, 1);
}

/**
 * @brief CG_ShoutcasterMinusRedScore
 */
static void CG_ShoutcasterMinusRedScore(void)
{
	CG_ShoutcastAddScore(TEAM_AXIS, -1);
}

/**
 * @brief CG_ShoutcasterResetRedScore
 */
static void CG_ShoutcasterResetRedScore(void)
{
	CG_ShoutcastAddScore(TEAM_AXIS, -cg_shoutcastTeamScoreRed.integer);
}

/**
 * @brief CG_ShoutcasterPlusBlueScore
 */
static void CG_ShoutcasterPlusBlueScore(void)
{
	CG_ShoutcastAddScore(TEAM_ALLIES, 1);
}

/**
 * @brief CG_ShoutcasterMinusBlueScore
 */
static void CG_ShoutcasterMinusBlueScore(void)
{
	CG_ShoutcastAddScore(TEAM_ALLIES, -1);
}

/**
 * @brief CG_ShoutcasterResetBlueScore
 */
static void CG_ShoutcasterResetBlueScore(void)
{
	CG_ShoutcastAddScore(TEAM_ALLIES, -cg_shoutcastTeamScoreBlue.integer);
}

/**
 * @brief CG_ShoutcasterSwapTeamLabels
 */
static void CG_ShoutcasterSwapTeamLabels(void)
{
	char name[MAX_CVAR_VALUE_STRING];
	int  score = cg_shoutcastTeamScoreRed.integer;

	Q_strncpyz(name, cg_shoutcastTeamNameRed.string, MAX_CVAR_VALUE_STRING);

	// swap team name
	trap_Cvar_Set("cg_shoutcastTeamNameRed", cg_shoutcastTeamNameBlue.string);
	trap_Cvar_Set("cg_shoutcastTeamNameBlue", name);

	// swap team score
	trap_Cvar_Set("cg_shoutcastTeamScoreRed", va("%i", cg_shoutcastTeamScoreBlue.integer));
...
static consoleCommand_t commands[] =
{
	...
	// shoutcaster team and score handler
	{ "scPlusRedScore",         CG_ShoutcasterPlusRedScore   },
	{ "scMinusRedScore",        CG_ShoutcasterMinusRedScore  },
	{ "scResetRedScore",        CG_ShoutcasterResetRedScore  },
	{ "scPlusBlueScore",        CG_ShoutcasterPlusBlueScore  },
