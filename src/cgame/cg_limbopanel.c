...
void CG_LimboPanel_KeyHandling(int key, qboolean down)
{
	...
	if (down)
	{
		...
		if (BG_PanelButtons_GetFocusButton() == &weaponPanel)
		{
			...
			int selectableWeaponCount;

			for (selectableWeaponCount = 1; weaponPanel.data[selectableWeaponCount] != 0; selectableWeaponCount++)
			{
			}
			...
			if (selectedWeapon != 0 && selectedWeapon < selectableWeaponCount)
			{
				CG_LimboPanel_WeaponPanel_SelectButton(&weaponPanel, selectableWeaponCount - selectedWeapon);
				BG_PanelButtons_SetFocusButton(NULL);
			}
