...
static void CG_ServerCommand(void)
{
	...
	else if (!Q_stricmp(cmd, "cp"))
	{
		...
		if (args >= 3)
		{
			...
			CG_PriorityCenterPrint(s, SCREEN_HEIGHT * 0.8), SMALLCHAR_WIDTH, atoi(CG_Argv(2)));
		}
		else
		{
			CG_CenterPrint(CG_LocalizeServerCommand(CG_Argv(1)), SCREEN_HEIGHT * 0.8), SMALLCHAR_WIDTH);
		}
		return;
	}
