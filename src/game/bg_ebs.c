...
===========================================================================
*/
// CPMA 1.53 live stats for team overlays

#include "../qcommon/q_shared.h"
#include "bg_ebs.h"

// in total, an entityState_t struct provides:
// 435 generic bits, of which we use ebs_maxIntBits
//  24 floats
...
// we don't use trType not because we can't work around it,
// but to make sure older CPMA versions and q3mme can read the demos without a QVM drop error
...
static const intField_t entityIntFields[] =
{
	...
	// ESF(eFlags,          24),
	//ESF(event, 10),		// @NOTE: we can't override this! it messes up CPMA_SoundMarshal
	//ESF(pos.trType,      8), // @NOTE: avoiding cgame crashes when evaluating trajectories
	ESF(pos.trTime,      32),
...
const int ebs_maxIntBits = 644; //382;
...
// qboolean EBS_IsLiveStatsEntity(entityState_t *ent)
// {
// 	return ent->eType == ET_GENERAL && (ent->eFlags & EF_LIVE_STATS) != 0;
// }
