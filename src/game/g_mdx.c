...
struct frame_bone {
	...
	float anglesF[3];			// core: floating point values instead of short integers
	float offset_anglesF[2];	// core: floating point values instead of short integers
};
...
static void mdx_load(mdx_t *mdxModel, char *mem)
{
	...
	for (i = 0; i < mdxModel->frame_count; i++) {
		...
		for (j = 0; j < mdxModel->bone_count; j++) {
			...
			// core: convert short integer values to floating point values
...
static void mdx_bone_orientation(/*const*/ grefEntity_t *refent, int idx, vec3_t origin, vec3_t axis[3])
{
	...
	/* bone angles */
/*
	// core: original code is commented.
...
*/
	// core: removing the SHORT2ANGLE() calls.
