...
void G_FreeEntity(gentity_t *ed)
{
	...
	// - when g_debughitboxes is enabled we want visible trace effects - don't free immediately
	// FIXME: remove tmp var l_free if we are sure there are no issues caused by this change (especially on network games)
	if ((ed->s.eType == ET_TEMPHEAD || ed->s.eType == ET_TEMPLEGS || ed->s.eType >= ET_EVENTS) && trap_Cvar_VariableIntegerValue("l_free") == 0 && trap_Cvar_VariableIntegerValue("g_debughitboxes") == 0)
	{
		// debug
