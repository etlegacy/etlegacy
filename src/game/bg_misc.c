 ...
 * [9]   = isLWSF               - Light Weapon Supporting Fast Reload
 * [10]  = damage               - returns 1 for no damage, return 0 for no explode on contact ... FIXME: some weapons are handled differently f.e. VERYBIGEXPLOSION
 * [11]  = canGib               - 
 * [12]  = isReload             - some weapons don't reload
 * [13]  = spread               - 
 * [14]  = spreadScale          - 
 * [15]  = splashDamage         - 
 * [16]  = splashRadius         - 
 ...
 * [54]  = useClip              - 
 * [55]  = useBullet            - 
 * [56]  = maxAmmo              - max player ammo carrying capacity.
 * [57]  = uses                 - how many 'rounds' it takes/costs to fire one cycle.
 * [58]  = maxClip              - max 'rounds' in a clip.
 * [59]  = defaultStartingAmmo  - player ammo when spawning.
 * [60]  = defaultStartingClip  - player clips when spawning.
 * [61]  = reloadTime           - time from start of reload until ready to fire.
 * [62]  = fireDelayTime        - time from pressing 'fire' until first shot is fired. (used for delaying fire while weapon is 'readied' in animation)
 * [63]  = nextShotTime         - when firing continuously, this is the time between shots
 * [64]  = grenadeTime          - 
 * [65]  = aimSpreadScaleAdd    - 
 * [66]  = maxHeat              - max active firing time before weapon 'overheats' (at which point the weapon will fail for a moment)
 * [67]  = coolRate             - how fast the weapon cools down.
 * [68]  = heatRecoveryTime     - time from overheats until weapon can fire again
 * [69]  = switchTimeBegin      - 
 * [70]  = switchTimeFinish     - 
 * [71]  = altSwitchTimeBegin   - 
 * [72]  = altSwitchTimeFinish  - 
 * [73]  = knockback            - 
 * [74]  = ejectBrassOffset     - forward, left, up
 * [75]  = adjustLean           - 
 * [76]  = fireRecoilPitch      - 
 * [77]  = fireRecoilYaw        - 
 * [78]  = weapRecoilDuration   - 
 * [79]  = weapRecoilPitch      - 
 * [80]  = weapRecoilYaw        - 
 * [81]  = className            - 
 * [82]  = weapFile             - 
 ...
 * [92]  = useChargeTime        - 
 * [93]  = chargeTimeCoeff      - 
 * [94]  = mod                  - means of death.
 * [95]  = splashMod            - splash means of death.
 */
weaponTable_t weaponTable[WP_NUM_WEAPONS] =
{
	// weapon                  item                               team         skillBased                                   weapAlts                weapEquiv          akimboSidearm   ammoIndex             clipIndex             ammunType           isScoped isLWSF  damage canGib  isReload spread spreadScale splashDamage splashRadius quickFireMode firingAuto neverLoseDisguise keepDisguise isThrowable isAutoReload noAmmoSound noAmmoAutoSwitch isSMG   isExplosive isSyringe isPistol isAkimbo isPanzer isRiflenade isRifle isRifleWithScope isMortar isMortarSet isMG    isMGSet isSetWeapon isLightWeapon isHeavyWeapon isSilencedPistol isMeleeWeapon isGrenade underWaterFire   noMuzzleFlash shakeEffect zoomOut zoomIn zoomedScope desc                   indexWeaponStat     fallOff useAmmo useClip  useBullet maxAmmo uses maxClip defaultStartingAmmo defaultStartingClip reloadTime firstDelayTime nextShotTime grenadeTime aimSpreadScaleAdd maxHeat coolRate heatRecoveryTime switchTimeBegin switchTimeFinish altSwitchTimeBegin altswitchTimeFinish knockback ejectBrassOffset muzzlePointOffset adjustLean fireRecoilPitch fireRecoilYaw weapRecoilDuration weapRecoilPitch weapRecoilYaw  className          weapFile                 weaponCardIcon    weaponCardCoord                             idleAnim    attackAnim    lastAttackAnim        altSwitchFrom       altSwitchTo         reloadAnim    raiseAnim     dropAnim      useChargeTime chargeTimeCoeff                      mod                       splashMod
    { WP_NONE,                 ITEM_NONE,                         TEAM_FREE,   SK_NUM_SKILLS,                               WP_NONE,                WP_NONE,                 WP_NONE,  WP_NONE,              WP_NONE,              AMMUN_NONE,         qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qtrue,            qtrue,       qfalse,     qfalse,      qfalse,     qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qtrue,        qfalse,     0,      0,     0,          "WP_NONE",             WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   0,      0,   0,      0,                  0,                  0,         50,            0,           0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 0, 0, 0 },     { 0, 0, 0 },      1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                NULL,                    WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_NONE                  // 0
    { WP_KNIFE,                ITEM_WEAPON_KNIFE,                 TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_KNIFE_KABAR,          WP_NONE,  WP_KNIFE,             WP_KNIFE,             AMMUN_MELEE,        qfalse,  qfalse, 10,    qtrue,  qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qtrue,       qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qtrue,        qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "KNIFE",               WS_KNIFE,           qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    1,                  0,                  0,         50,            200,         0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "knife",                 WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_KNIFE,                MOD_KNIFE                },  // WP_KNIFE                 // 1
	{ WP_LUGER,                ITEM_WEAPON_LUGER,                 TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_SILENCER,            WP_COLT,                 WP_NONE,  WP_LUGER,             WP_LUGER,             AMMUN_BULLET,       qfalse,  qtrue,  18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qtrue,   qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "LUGER",               WS_LUGER,           qtrue,  qtrue,  qtrue,   qtrue,    24,     1,   8,      24,                 8,                  1500,      DELAY_PISTOL,  400,         0,          20,               0,      0,       0,               250,            250,             0,                 0,                  0,        { 24, -4, 36 },  { 14, 6, -4 },    1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "luger",                 WEAPON_CARD_2,    { 0.5f, 1.f, 0.f, 0.5f, 2 / 8.f, 3 / 8.f }, WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_LUGER,                MOD_LUGER                },  // WP_LUGER                 // 2    // NOTE: also 32 round 'snail' magazine
	{ WP_MP40,                 ITEM_WEAPON_MP40,                  TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_NONE,                WP_THOMPSON,             WP_NONE,  WP_MP40,              WP_MP40,              AMMUN_BULLET,       qfalse,  qtrue,  18,    qfalse, qtrue,   400,   0.6f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qtrue,  qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MP 40",               WS_MP40,            qtrue,  qtrue,  qtrue,   qtrue,    90,     1,   30,     30,                 30,                 2400,      DELAY_LOW,     150,         0,          15,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "mp40",                  WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 5 / 8.f, 6 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MP40,                 MOD_MP40                 },  // WP_MP40                  // 3
	{ WP_GRENADE_LAUNCHER,     ITEM_WEAPON_GRENADE_LAUNCHER,      TEAM_AXIS,   SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_NONE,                WP_GRENADE_PINEAPPLE,    WP_NONE,  WP_GRENADE_LAUNCHER,  WP_GRENADE_LAUNCHER,  AMMUN_GRENADE,      qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         250,         qfalse,       qfalse,    qfalse,           qtrue,       qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qtrue,    qtrue,           qtrue,        qtrue,      0,      0,     0,          "",                    WS_GRENADE,         qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_THROW,   1600,        4000,       0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 20, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "grenade",         "grenade",               WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_GRENADE_LAUNCHER,     MOD_GRENADE_LAUNCHER     },  // WP_GRENADE_LAUNCHER      // 4
	{ WP_PANZERFAUST,          ITEM_WEAPON_PANZERFAUST,           TEAM_AXIS,   SK_HEAVY_WEAPONS,                            WP_NONE,                WP_BAZOOKA,              WP_NONE,  WP_PANZERFAUST,       WP_PANZERFAUST,       AMMUN_ROCKET,       qfalse,  qfalse, 400,   qtrue,  qfalse,  0,     0,          400,         300,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qtrue,   qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qtrue,      0,      0,     0,          "PANZERFAUST",         WS_PANZERFAUST,     qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_HW,      2000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                32000.f,  { -24, -4, 24 }, { 14, 10, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "rocket",          "panzerfaust",           WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 1 / 8.f, 2 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, .66f, .66f, .66f, .66f},         MOD_PANZERFAUST,          MOD_PANZERFAUST          },  // WP_PANZERFAUST           // 5    // updated delay so prediction is correct
	{ WP_FLAMETHROWER,         ITEM_WEAPON_FLAMETHROWER,          TEAM_FREE,   SK_HEAVY_WEAPONS,                            WP_NONE,                WP_NONE,                 WP_NONE,  WP_FLAMETHROWER,      WP_FLAMETHROWER,      AMMUN_FLAME,        qfalse,  qfalse, 5,     qfalse, qfalse,  0,     0,          5,           5,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "FLAMETHROWER",        WS_FLAMETHROWER,    qfalse, qtrue,  qfalse,  qfalse,   200,    1,   200,    200,                0,                  0,         DELAY_LOW,     50,          0,          0,                0,      0,       0,               250,            250,             250,               250,                2000.f,   { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "flamechunk",      "flamethrower",          WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 2 / 8.f, 3 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_FLAMETHROWER,         MOD_FLAMETHROWER         },  // WP_FLAMETHROWER          // 6
	{ WP_COLT,                 ITEM_WEAPON_COLT,                  TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_SILENCED_COLT,       WP_LUGER,                WP_NONE,  WP_COLT,              WP_COLT,              AMMUN_BULLET,       qfalse,  qtrue,  18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qtrue,   qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "COLT",                WS_COLT,            qtrue,  qtrue,  qtrue,   qtrue,    24,     1,   8,      24,                 8,                  1500,      DELAY_PISTOL,  400,         0,          20,               0,      0,       0,               250,            250,             0,                 0,                  0,        { 24, -4, 36 },  { 14, 6, -4 },    1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "colt",                  WEAPON_CARD_2,    { 0.5f, 1.f, 0.f, 0.5f, 3 / 8.f, 4 / 8.f }, WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_COLT,                 MOD_COLT                 },  // WP_COLT                  // 7    // equivalent american weapon to german luger
	{ WP_THOMPSON,             ITEM_WEAPON_THOMPSON,              TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_MP40,                 WP_NONE,  WP_THOMPSON,          WP_THOMPSON,          AMMUN_BULLET,       qfalse,  qtrue,  18,    qfalse, qtrue,   400,   0.6f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qtrue,  qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "THOMPSON",            WS_THOMPSON,        qtrue,  qtrue,  qtrue,   qtrue,    90,     1,   30,     30,                 30,                 2400,      DELAY_LOW,     150,         0,          15,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "thompson",              WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 7 / 8.f, 8 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_THOMPSON,             MOD_THOMPSON             },  // WP_THOMPSON              // 8    // equivalent american weapon to german mp40
	{ WP_GRENADE_PINEAPPLE,    ITEM_WEAPON_GRENADE_PINEAPPLE,     TEAM_ALLIES, SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_NONE,                WP_GRENADE_LAUNCHER,     WP_NONE,  WP_GRENADE_PINEAPPLE, WP_GRENADE_PINEAPPLE, AMMUN_GRENADE,      qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         250,         qfalse,       qfalse,    qfalse,           qtrue,       qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qtrue,    qtrue,           qtrue,        qtrue,      0,      0,     0,          "",                    WS_GRENADE,         qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_THROW,   1600,        4000,       0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 20, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "grenade",         "pineapple",             WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_GRENADE_PINEAPPLE,    MOD_GRENADE_PINEAPPLE    },  // WP_GRENADE_PINEAPPLE     // 9
    //                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
	{ WP_STEN,                 ITEM_WEAPON_STEN,                  TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_MP34,                 WP_NONE,  WP_STEN,              WP_STEN,              AMMUN_BULLET,       qfalse,  qtrue,  14,    qfalse, qtrue,   200,   0.6f,       0,           0,           qfalse,       qtrue,     qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "STEN",                WS_STEN,            qtrue,  qtrue,  qtrue,   qtrue,    96,     1,   32,     32,                 32,                 3100,      DELAY_LOW,     150,         0,          15,               1200,   450,     2000,            250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "sten",                  WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 6 / 8.f, 7 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_STEN,                 MOD_STEN                 },  // WP_STEN                  // 10   // silenced sten sub-machinegun
	{ WP_MEDIC_SYRINGE,        ITEM_WEAPON_MEDIC_SYRINGE,         TEAM_FREE,   SK_FIRST_AID,                                WP_NONE,                WP_NONE,                 WP_NONE,  WP_MEDIC_SYRINGE,     WP_MEDIC_SYRINGE,     AMMUN_SYRINGUE,     qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qfalse,     qtrue,           qfalse, qfalse,     qtrue,    qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "MEDIC",               WS_MAX,             qfalse, qtrue,  qfalse,  qfalse,   10,     1,   1,      10,                 0,                  0,         50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "syringe",               WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_SYRINGE,              MOD_SYRINGE              },  // WP_MEDIC_SYRINGE         // 11   // broken out from CLASS_SPECIAL per Id request
	{ WP_AMMO,                 ITEM_WEAPON_MAGICAMMO,             TEAM_FREE,   SK_SIGNALS,                                  WP_NONE,                WP_NONE,                 WP_NONE,  WP_AMMO,              WP_AMMO,              AMMUN_BOX,          qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "AMMO",                WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   1,      0,   1,      0,                  1,                  3000,      50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "ammopack",              WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {0.25f, 0.15f, 0.15f, 0.15f, 0.15f}, MOD_AMMO,                 MOD_AMMO                 },  // WP_AMMO                  // 12   // likewise
	{ WP_ARTY,                 ITEM_WEAPON_ARTY,                  TEAM_FREE,   SK_SIGNALS,                                  WP_NONE,                WP_NONE,                 WP_NONE,  WP_ARTY,              WP_ARTY,              AMMUN_ARTY,         qfalse,  qfalse, 1,     qtrue,  qfalse,  0,     0,          400,         400,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qtrue,      0,      0,     0,          "ARTY",                WS_ARTILLERY,       qfalse, qfalse, qfalse,  qfalse,   1,      0,   1,      0,                  1,                  3000,      50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "air strike",      "arty",                  WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, .66f, .66f, .66f},            MOD_ARTY,                 MOD_ARTY                 },  // WP_ARTY                  // 13
	{ WP_SILENCER,             ITEM_WEAPON_SILENCER,              TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_LUGER,               WP_SILENCED_COLT,        WP_NONE,  WP_LUGER,             WP_LUGER,             AMMUN_BULLET,       qfalse,  qtrue,  18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qtrue,           qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SILENCED LUGER",      WS_LUGER,           qtrue,  qtrue,  qtrue,   qtrue,    24,     1,   8,      24,                 8,                  1500,      DELAY_PISTOL,  400,         0,          20,               0,      0,       0,               250,            250,             1000,              1190,               0,        { 24, -4, 36 },  { 14, 6, -4 },    1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "silenced_luger",        WEAPON_CARD_2,    { 0.5f, 1.f, 0.f, 0.5f, 0 / 8.f, 1 / 8.f }, WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_SILENCER,             MOD_SILENCER             },  // WP_SILENCER              // 14   // used to be sp5
	{ WP_DYNAMITE,             ITEM_WEAPON_DYNAMITE,              TEAM_FREE,   SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_NONE,                WP_NONE,                 WP_NONE,  WP_DYNAMITE,          WP_DYNAMITE,          AMMUN_DYNAMITE,     qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          400,         400,         qfalse,       qfalse,    qfalse,           qfalse,      qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qtrue,      0,      0,     0,          "DYNAMITE",            WS_DYNAMITE,        qfalse, qfalse, qfalse,  qfalse,   1,      0,   1,      0,                  1,                  1000,      DELAY_THROW,   1600,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 20, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "dynamite",        "dynamite",              WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, 1, .66f, .66f},               MOD_DYNAMITE,             MOD_DYNAMITE             },  // WP_DYNAMITE              // 15
	{ WP_SMOKETRAIL,           ITEM_WEAPON_SMOKETRAIL,            TEAM_FREE,   SK_SIGNALS,                                  WP_NONE,                WP_NONE,                 WP_NONE,  WP_SMOKETRAIL,        WP_SMOKETRAIL,        AMMUN_AIRSTRIKE,    qfalse,  qfalse, 0,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "",                    WS_ARTILLERY,       qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    0,                  0,                  0,         50,            0,           0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "WP",              "smoketrail",            WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_SMOKETRAIL            // 16   // WP == White Phosphorous, so we can check for bounce noise in grenade bounce routine
	{ WP_MAPMORTAR,            ITEM_WEAPON_MAPMORTAR,             TEAM_FREE,   SK_HEAVY_WEAPONS,                            WP_NONE,                WP_NONE,                 WP_NONE,  WP_MAPMORTAR,         WP_MAPMORTAR,         AMMUN_MAPMORTAR,    qfalse,  qfalse, 250,   qtrue,  qfalse,  0,     0,          250,         120,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qtrue,      0,      0,     0,          "",                    WS_MORTAR,          qfalse, qtrue,  qfalse,  qfalse,   999,    0,   999,    0,                  0,                  0,         50,            0,           0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "mortar",          "mapmortar",             WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_MAPMORTAR             // 17
	{ VERYBIGEXPLOSION,        ITEM_NONE,                         TEAM_FREE,   SK_NUM_SKILLS,                               WP_NONE,                WP_NONE,                 WP_NONE,  WP_NONE,              WP_NONE,              AMMUN_NONE,         qfalse,  qfalse, 1,     qtrue,  qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "",                    WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    0,                  0,                  0,         50,            0,           0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                NULL,                    WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // VERYBIGEXPLOSION         // 18   // explosion effect for airplanes
	{ WP_MEDKIT,               ITEM_WEAPON_MEDIC_HEAL,            TEAM_FREE,   SK_FIRST_AID,                                WP_NONE,                WP_NONE,                 WP_NONE,  WP_MEDKIT,            WP_MEDKIT,            AMMUN_BOX,          qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "",                    WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    1,                  1,                  0,         50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "medpack",               WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {0.25f, 0.25f, 0.15f, 0.15f, 0.15f}, MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_MEDKIT                // 19
    //                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
	{ WP_BINOCULARS,           ITEM_WEAPON_BINOCULARS,            TEAM_FREE,   SK_NUM_SKILLS,                               WP_NONE,                WP_NONE,                 WP_NONE,  WP_BINOCULARS,        WP_BINOCULARS,        AMMUN_NONE,         qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qtrue,            qtrue,       qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qfalse,       qfalse,     20/*36*/, 4/*8*/,     0,   "",                    WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    1,                  0,                  0,         50,            0,           0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "binocs",                WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_BINOCULARS            // 20   // per atvi request all use same vals to match menu {36 8}
	{ WP_PLIERS,               ITEM_WEAPON_PLIERS,                TEAM_FREE,   SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_NONE,                WP_NONE,                 WP_NONE,  WP_PLIERS,            WP_PLIERS,            AMMUN_NONE,         qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "PLIERS",              WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    0,                  1,                  0,         50,            50,          0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "pliers",                WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_PLIERS                // 21
	{ WP_SMOKE_MARKER,         ITEM_WEAPON_SMOKE_MARKER,          TEAM_FREE,   SK_SIGNALS,                                  WP_NONE,                WP_NONE,                 WP_NONE,  WP_SMOKE_MARKER,      WP_SMOKE_MARKER,      AMMUN_CANISTER,     qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          400,         400,         qfalse,       qtrue,     qfalse,           qfalse,      qtrue,      qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qtrue,      0,      0,     0,          "",                    WS_AIRSTRIKE,       qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    0,                  1,                  0,         50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "grenade",         "smokemarker",           WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, .66f, .66f, .66f},            MOD_AIRSTRIKE,            MOD_AIRSTRIKE            },  // WP_SMOKE_MARKER          // 22   // changed name to cause less confusion // damage was 140
	{ WP_KAR98,                ITEM_WEAPON_KAR98,                 TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_GPG40,               WP_CARBINE,              WP_NONE,  WP_KAR98,             WP_KAR98,             AMMUN_BULLET,       qfalse,  qfalse, 34,    qfalse, qtrue,   250,   0.5f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qtrue,  qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "K43",                 WS_KAR98,           qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      DELAY_LOW,     400,         0,          50,               0,      0,       0,               250,            250,             0,                 1347,               0,        { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      2.f,            1.f,          0,                 { 0, 0 },       { 0, 0 },      "",                "kar98",                 WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 5 / 8.f, 6 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_KAR98,                MOD_KAR98                },  // WP_KAR98                 // 23   // WolfXP weapons    K43
	{ WP_CARBINE,              ITEM_WEAPON_CARBINE,               TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_M7,                  WP_KAR98,                WP_NONE,  WP_CARBINE,           WP_CARBINE,           AMMUN_BULLET,       qfalse,  qfalse, 34,    qfalse, qtrue,   250,   0.5f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qtrue,  qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "M1 GARAND",           WS_CARBINE,         qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      DELAY_LOW,     400,         0,          50,               0,      0,       0,               250,            250,             0,                 1347,               0,        { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      2.f,            1.f,          0,                 { 0, 0 },       { 0, 0 },      "",                "m1_garand",             WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 4 / 8.f, 5 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_CARBINE,              MOD_CARBINE              },  // WP_CARBINE               // 24   // GARAND old max ammo 24 max clip size 8 start ammo 16 start clip 8
	{ WP_GARAND,               ITEM_WEAPON_GARAND,                TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_GARAND_SCOPE,        WP_K43,                  WP_NONE,  WP_GARAND,            WP_GARAND,            AMMUN_BULLET,       qfalse,  qfalse, 34,    qfalse, qtrue,   250,   0.5f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qtrue,           qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SCOPED M1 GARAND",    WS_GARAND,          qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      DELAY_LOW,     400,         0,          50,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    3.0f,      2.f,            1.f,          0,                 { 0, 0 },       { 0, 0 },      "",                "m1_garand_s",           WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 7 / 8.f, 8 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_GARAND,               MOD_GARAND               },  // WP_GARAND                // 25   // GARAND old max ammo 24 max clip size 8 start ammo 16 start clip 8
	{ WP_LANDMINE,             ITEM_WEAPON_LANDMINE,              TEAM_FREE,   SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_NONE,                WP_NONE,                 WP_NONE,  WP_LANDMINE,          WP_LANDMINE,          AMMUN_LANDMINE,     qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         225,         qfalse,       qfalse,    qfalse,           qfalse,      qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qtrue,      0,      0,     0,          "",                    WS_LANDMINE,        qfalse, qtrue,  qfalse,  qfalse,   1,      0,   1,      0,                  1,                  100,       DELAY_LOW,     100,         0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "landmine",        "landmine",              WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {.5f, .5f, .5f, .33f, .33f},         MOD_LANDMINE,             MOD_LANDMINE             },  // WP_LANDMINE              // 26   // splashRadius was: 400 // for charTime use 33%, not 66%, when upgraded. do not penalize the happy fun engineer.
	{ WP_SATCHEL,              ITEM_WEAPON_SATCHEL,               TEAM_FREE,   SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, WP_NONE,                WP_NONE,                 WP_NONE,  WP_SATCHEL,           WP_SATCHEL,           AMMUN_SATCHEL,      qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         250,         qfalse,       qfalse,    qtrue,            qfalse,      qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qtrue,        qfalse,     0,      0,     0,          "SATCHEL",             WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   1,      0,   1,      0,                  1,                  3000,      DELAY_LOW,     0/*2000*/,   0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 20, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "satchel_charge",  "satchel",               WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, .66f, .66f, .66f},            MOD_SATCHEL,              MOD_SATCHEL              },  // WP_SATCHEL               // 27
	{ WP_SATCHEL_DET,          ITEM_WEAPON_SATCHELDET,            TEAM_FREE,   SK_NUM_SKILLS,                               WP_NONE,                WP_NONE,                 WP_NONE,  WP_SATCHEL_DET,       WP_SATCHEL_DET,       AMMUN_BULLET,       qfalse,  qfalse, 1,     qtrue,  qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qtrue,            qtrue,       qfalse,     qfalse,      qfalse,     qtrue,           qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qtrue,        qtrue,      0,      0,     0,          "SATCHEL",             WS_SATCHEL,         qfalse, qfalse, qfalse,  qfalse,   1,      0,   1,      0,                  0,                  3000,      722,           0/*2000*/,   0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "satchel_det",           WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RELOAD2, WEAP_RELOAD1, qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_SATCHEL_DET           // 28
	{ WP_SMOKE_BOMB,           ITEM_WEAPON_SMOKE_BOMB,            TEAM_FREE,   SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, WP_NONE,                WP_NONE,                 WP_NONE,  WP_SMOKE_BOMB,        WP_SMOKE_BOMB,        AMMUN_SMOKEGRENADE, qfalse,  qfalse, 0,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qtrue,            qtrue,       qtrue,      qfalse,      qfalse,     qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "",                    WS_MAX,             qfalse, qfalse, qfalse,  qfalse,   1,      0,   10,     0,                  1,                  1000,      DELAY_THROW,   1600,        4000,       0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 20, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "smoke_bomb",      "smokegrenade",          WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, .66f, .66f, .66f},            MOD_SMOKEBOMB,            MOD_SMOKEBOMB            },  // WP_SMOKE_BOMB            // 29
    //                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
	{ WP_MOBILE_MG42,          ITEM_WEAPON_MOBILE_MG42,           TEAM_AXIS,   SK_HEAVY_WEAPONS,                            WP_MOBILE_MG42_SET,     WP_MOBILE_BROWNING,      WP_NONE,  WP_MOBILE_MG42,       WP_MOBILE_MG42,       AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   2500,  0.9f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qtrue,  qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MOBILE MG 42",        WS_MG42,            qfalse, qtrue,  qtrue,   qtrue,    450,    1,   150,    0,                  150,                3000,      DELAY_LOW,     66,          0,          20,               1500,   300,     2000,            250,            250,             0,                 1722,               4000.f,   { 12, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         200,               { .75f, .2f },  { 1.f, .25f }, "",                "mg42",                  WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 4 / 8.f, 5 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MOBILE_MG42,          MOD_MOBILE_MG42          },  // WP_MOBILE_MG42           // 30
    { WP_K43,                  ITEM_WEAPON_K43,                   TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_K43_SCOPE,           WP_GARAND,               WP_NONE,  WP_K43,               WP_K43,               AMMUN_BULLET,       qfalse,  qfalse, 34,    qfalse, qtrue,   250,   0.5f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qtrue,           qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SCOPED K43",          WS_K43,             qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      DELAY_LOW,     400,         0,          50,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      2.f,            1.f,          0,                 { 0, 0 },       { 0, 0 },      "",                "k43",                   WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 6 / 8.f, 7 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_K43,                  MOD_K43                  },  // WP_K43                   // 31    // K43
	{ WP_FG42,                 ITEM_WEAPON_FG42,                  TEAM_FREE,   SK_LIGHT_WEAPONS,                            WP_FG42SCOPE,           WP_NONE,                 WP_NONE,  WP_FG42,              WP_FG42,              AMMUN_BULLET,       qfalse,  qtrue,  16,    qfalse, qtrue,   500,   0.6f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "FG 42",               WS_FG42,            qtrue,  qtrue,  qtrue,   qtrue,    60,     1,   20,     40,                 20,                 2000,      DELAY_LOW,     100,         0,          100,              0,      0,       0,               250,            250,             50,                50,                 0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "fg42",                  WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 3 / 8.f, 4 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_FG42,                 MOD_FG42                 },  // WP_FG42                  // 32
	{ WP_DUMMY_MG42,           ITEM_WEAPON_DUMMY_MG42,            TEAM_FREE,   SK_NUM_SKILLS,                               WP_NONE,                WP_NONE,                 WP_NONE,  WP_DUMMY_MG42,        WP_DUMMY_MG42,        AMMUN_BULLET,       qfalse,  qfalse, 20,    qfalse, qfalse,  100,   0.9f,       0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qtrue,  qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "",                    WS_MG42,            qfalse, qfalse, qfalse,  qfalse,   0,      0,   0,      0,                  0,                  0,         0,             66,          0,          0,                1500,   300,     2000,            250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                NULL,                    WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_UNKNOWN,              MOD_UNKNOWN              },  // WP_DUMMY_MG42            // 33    // for storing heat on mounted mg42s...
	{ WP_MORTAR,               ITEM_WEAPON_MORTAR,                TEAM_ALLIES, SK_HEAVY_WEAPONS,                            WP_MORTAR_SET,          WP_MORTAR2,              WP_NONE,  WP_MORTAR,            WP_MORTAR,            AMMUN_NONE,         qfalse,  qfalse, 1,     qtrue,  qtrue,   0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qtrue,   qfalse,     qfalse, qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MORTAR",              WS_MORTAR,          qfalse, qtrue,  qfalse,  qfalse,   15,     1,   1,      15,                 0,                  0,         DELAY_HW,      1600,        0,          0,                0,      0,       0,               250,            250,             0,                 1000,               0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "mortar",                WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 0 / 8.f, 1 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MORTAR,               MOD_MORTAR               },  // WP_MORTAR                // 34
	{ WP_AKIMBO_COLT,          ITEM_WEAPON_AKIMBO_COLT,           TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_AKIMBO_LUGER,         WP_COLT,  WP_COLT,              WP_AKIMBO_COLT,       AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qtrue,   qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "AKIMBO COLTS",        WS_COLT,            qtrue,  qtrue,  qtrue,   qtrue,    48,     1,   8,      48,                 8,                  2700,      DELAY_PISTOL,  200,         0,          20,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, -6, -4 },   1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "akimbo_colt",           WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 3 / 8.f, 4 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_AKIMBO_COLT ,         MOD_AKIMBO_COLT          },  // WP_AKIMBO_COLT           // 35
	{ WP_AKIMBO_LUGER,         ITEM_WEAPON_AKIMBO_LUGER,          TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_NONE,                WP_AKIMBO_COLT,          WP_LUGER, WP_LUGER,             WP_AKIMBO_LUGER,      AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qtrue,   qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "AKIMBO LUGERS",       WS_LUGER,           qtrue,  qtrue,  qtrue,   qtrue,    48,     1,   8,      48,                 8,                  2700,      DELAY_PISTOL,  200,         0,          20,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, -6, -4 },   1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "akimbo_luger",          WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 2 / 8.f, 3 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_AKIMBO_LUGER,         MOD_AKIMBO_LUGER         },  // WP_AKIMBO_LUGER          // 36
    //                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  
	{ WP_GPG40,                ITEM_WEAPON_GPG40,                 TEAM_AXIS,   SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_KAR98,               WP_NONE,                 WP_NONE,  WP_GPG40,             WP_GPG40,             AMMUN_RIFLENADE,    qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         250,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qtrue,      qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qtrue,        qfalse,     0,      0,     0,          "",                    WS_GRENADELAUNCHER, qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_LOW,     400,         0,          0,                0,      0,       0,               250,            250,             0,                 2350,               0,        { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "gpg40_grenade",   "gpg40",                 WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK2,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD2, WEAP_RELOAD3, WEAP_DROP2,   qtrue,        {.5f, .5f, .5f, .5f, .5f},           MOD_GPG40,                MOD_GPG40                },  // WP_GPG40                 // 37
    { WP_M7,                   ITEM_WEAPON_M7,                    TEAM_ALLIES, SK_EXPLOSIVES_AND_CONSTRUCTION,              WP_CARBINE,             WP_NONE,                 WP_NONE,  WP_M7,                WP_M7,                AMMUN_RIFLENADE,    qfalse,  qfalse, 0,     qtrue,  qfalse,  0,     0,          250,         250,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qtrue,      qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qtrue,        qfalse,     0,      0,     0,          "",                    WS_GRENADELAUNCHER, qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_LOW,     400,         0,          0,                0,      0,       0,               250,            250,             0,                 2350,               0,        { 16, -4, 24 },  { 14, 6, -4 },    2.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "m7_grenade",      "m7",                    WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK2,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD2, WEAP_RELOAD3, WEAP_DROP2,   qtrue,        {.5f, .5f, .5f, .5f, .5f},           MOD_M7,                   MOD_M7                   },  // WP_M7                    // 38
	{ WP_SILENCED_COLT,        ITEM_WEAPON_SILENCED_COLT,         TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_COLT,                WP_SILENCER,             WP_NONE,  WP_COLT,              WP_COLT,              AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qtrue,           qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SILENCED COLT",       WS_COLT,            qtrue,  qtrue,  qtrue,   qtrue,    24,     1,   8,      24,                 8,                  1500,      DELAY_PISTOL,  400,         0,          20,               0,      0,       0,               250,            250,             1000,              1190,               0,        { 24, -4, 36 },  { 14, 6, -4 },    1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "silenced_colt",         WEAPON_CARD_2,    { 0.5f, 1.f, 0.f, 0.5f, 1 / 8.f, 2 / 8.f }, WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_SILENCED_COLT,        MOD_SILENCED_COLT        },  // WP_SILENCED_COLT         // 39
    //                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
	{ WP_GARAND_SCOPE,         ITEM_WEAPON_GARAND_SCOPE,          TEAM_ALLIES, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, WP_GARAND,              WP_K43_SCOPE,            WP_NONE,  WP_GARAND,            WP_GARAND,            AMMUN_BULLET,       qtrue,   qfalse, 50,    qfalse, qtrue,   700,   10.f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     20,     4,     900,        "",                    WS_GARAND,          qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      0,             400,         0,          200,              0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0,            300,               { 0, 0 },       { 1.f, .5f },  "",                "m1_garand_s",           WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_GARAND_SCOPE,         MOD_GARAND_SCOPE         },  // WP_GARAND_SCOPE          // 40    // GARAND  old max ammo 24 max clip size 8 start ammo 16 start clip 8
    { WP_K43_SCOPE,            ITEM_WEAPON_K43_SCOPE,             TEAM_AXIS,   SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, WP_K43,                 WP_GARAND_SCOPE,         WP_NONE,  WP_K43,               WP_K43,               AMMUN_BULLET,       qtrue,   qfalse, 50,    qfalse, qtrue,   700,   10.f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     20,     4,     900,        "",                    WS_K43,             qfalse, qtrue,  qtrue,   qtrue,    30,     1,   10,     20,                 10,                 1500,      0,             400,         0,          200,              0,      0,       0,               250,            250,             250,               50,                 0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0,            300,               { 0, 0 },       { 1.f, .5f },  "",                "k43",                   WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_K43_SCOPE,            MOD_K43_SCOPE            },  // WP_K43_SCOPE             // 41    // K43
	{ WP_FG42SCOPE,            ITEM_WEAPON_FG42SCOPE,             TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_FG42,                WP_NONE,                 WP_NONE,  WP_FG42,              WP_FG42,              AMMUN_BULLET,       qtrue,   qfalse, 30,    qfalse, qtrue,   200,   10.f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     55,     55,    1,          "FG 42",               WS_FG42,            qfalse, qtrue,  qtrue,   qtrue,    60,     1,   20,     40,                 20,                 2000,      DELAY_LOW,     400,         0,          100,              0,      0,       0,               250,            250,             50,                250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         100,               { .45f, .15f }, { 0, 0 },      "",                "fg42",                  WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_FG42SCOPE,            MOD_FG42SCOPE            },  // WP_FG42SCOPE             // 42
	{ WP_MORTAR_SET,           ITEM_WEAPON_MORTAR_SET,            TEAM_ALLIES, SK_HEAVY_WEAPONS,                            WP_MORTAR,              WP_MORTAR2_SET,          WP_NONE,  WP_MORTAR,            WP_MORTAR,            AMMUN_SHELL,        qfalse,  qfalse, 250,   qtrue,  qtrue,   0,     0,          400,         400,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qtrue,      qfalse, qfalse, qtrue,      qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MORTAR",              WS_MORTAR,          qfalse, qtrue,  qfalse,  qfalse,   16,     1,   1,      16,                 0,                  0,         DELAY_HW,      1400,        0,          0,                0,      0,       0,               250,            250,             0,                 1667,               0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "mortar_grenade",  "mortar_set",            WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK1, WEAP_ATTACK1,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {.5f, .33f, .33f, .33f, .33f},       MOD_MORTAR,               MOD_MORTAR               },  // WP_MORTAR_SET            // 43
	{ WP_MEDIC_ADRENALINE,     ITEM_WEAPON_MEDIC_ADRENALINE,      TEAM_FREE,   SK_FIRST_AID,                                WP_NONE,                WP_NONE,                 WP_NONE,  WP_MEDIC_SYRINGE,     WP_MEDIC_SYRINGE,     AMMUN_SYRINGUE,     qfalse,  qfalse, 1,     qfalse, qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qtrue,       qfalse,     qfalse,      qfalse,     qtrue,           qfalse, qfalse,     qtrue,    qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qfalse,       qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "ADRENALINE",          WS_MAX,             qfalse, qtrue,  qfalse,  qfalse,   10,     1,   1,      10,                 0,                  1500,      50,            1000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "adrenaline",            WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, 1, 1, 1, 1},                     MOD_SYRINGE,              MOD_SYRINGE              },  // WP_MEDIC_ADRENALINE      // 44
	{ WP_AKIMBO_SILENCEDCOLT,  ITEM_WEAPON_AKIMBO_SILENCED_COLT,  TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_AKIMBO_SILENCEDLUGER, WP_COLT,  WP_COLT,              WP_AKIMBO_COLT,       AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qtrue,   qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SLNCD AKIMBO COLTS",  WS_COLT,            qtrue,  qtrue,  qtrue,   qtrue,    48,     1,   8,      48,                 8,                  2700,      DELAY_PISTOL,  200,         0,          20,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, -6, -4 },   1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "akimbo_silenced_colt",  WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 1 / 8.f, 2 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_AKIMBO_SILENCEDCOLT,  MOD_AKIMBO_SILENCEDCOLT  },  // WP_AKIMBO_SILENCEDCOLT   // 45
	{ WP_AKIMBO_SILENCEDLUGER, ITEM_WEAPON_AKIMBO_SILENCED_LUGER, TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_NONE,                WP_AKIMBO_SILENCEDCOLT,  WP_LUGER, WP_LUGER,             WP_AKIMBO_LUGER,      AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   600,   0.4f,       0,           0,           qtrue,        qfalse,    qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qtrue,   qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "SLNCD AKIMBO LUGERS", WS_LUGER,           qtrue,  qtrue,  qtrue,   qtrue,    48,     1,   8,      48,                 8,                  2700,      DELAY_PISTOL,  200,         0,          20,               0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, -6, -4 },   1.0f,      0,              0,            100,               { .45f, .15f }, { 0, 0 },      "",                "akimbo_silenced_luger", WEAPON_CARD_2,    { 1.f, 1.f, 0.f, 1.f, 0 / 8.f, 1 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_AKIMBO_SILENCEDLUGER, MOD_AKIMBO_SILENCEDLUGER },  // WP_AKIMBO_SILENCEDLUGER  // 46
	{ WP_MOBILE_MG42_SET,      ITEM_WEAPON_MOBILE_MG42_SET,       TEAM_AXIS,   SK_HEAVY_WEAPONS,                            WP_MOBILE_MG42,         WP_MOBILE_BROWNING_SET,  WP_NONE,  WP_MOBILE_MG42,       WP_MOBILE_MG42,       AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   2500,  0.9f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qtrue,           qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qtrue,  qtrue,      qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     55,     55,     0,         "MOBILE MG 42",        WS_MG42,            qfalse, qtrue,  qtrue,   qtrue,    450,    1,   150,    0,                  150,                3000,      DELAY_LOW,     66,          0,          20,               1500,   300,     2000,            250,            250,             0,                 1250,               0,        { 12, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "mg42",                  WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK2,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD3, WEAP_DROP2,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MOBILE_MG42,          MOD_MOBILE_MG42          },  // WP_MOBILE_MG42_SET       // 47
	// legacy weapons                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
	{ WP_KNIFE_KABAR,          ITEM_WEAPON_KNIFE_KABAR,           TEAM_ALLIES, SK_LIGHT_WEAPONS,                            WP_NONE,                WP_KNIFE,                WP_NONE,  WP_KNIFE_KABAR,       WP_KNIFE_KABAR,       AMMUN_MELEE,        qfalse,  qfalse, 10,    qtrue,  qfalse,  0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qtrue,       qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qfalse,       qfalse,          qtrue,        qfalse,   qtrue,           qtrue,        qfalse,     0,      0,     0,          "KABAR",               WS_KNIFE_KBAR,      qfalse, qfalse, qfalse,  qfalse,   999,    0,   999,    1,                  0,                  0,         50,            200,         0,          0,                0,      0,       0,               250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "knife_kbar",            WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_KNIFE_KABAR,          MOD_KNIFE_KABAR          },  // WP_KNIFE_KABAR           // 48
	{ WP_MOBILE_BROWNING,      ITEM_WEAPON_MOBILE_BROWNING,       TEAM_ALLIES, SK_HEAVY_WEAPONS,                            WP_MOBILE_BROWNING_SET, WP_MOBILE_MG42,          WP_NONE,  WP_MOBILE_BROWNING,   WP_MOBILE_BROWNING,   AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   2500,  0.9f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qtrue,  qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MOBILE BROWNING",     WS_BROWNING,        qfalse, qtrue,  qtrue,   qtrue,    450,    1,   150,    0,                  150,                3000,      DELAY_LOW,     66,          0,          20,               1500,   300,     2000,            250,            250,             0,                 1722,               4000.f,   { 12, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         200,               { .75f, .2f },  { 1.f, .25f }, "",                "browning",              WEAPON_CARD_3,    { 1.f, 1.f, 0.f, 1.f, 0 / 8.f, 1 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MOBILE_BROWNING,      MOD_MOBILE_BROWNING      },  // WP_MOBILE_BROWNING       // 49
	{ WP_MOBILE_BROWNING_SET,  ITEM_WEAPON_MOBILE_BROWNING_SET,   TEAM_ALLIES, SK_HEAVY_WEAPONS,                            WP_MOBILE_BROWNING,     WP_MOBILE_MG42_SET,      WP_NONE,  WP_MOBILE_BROWNING,   WP_MOBILE_BROWNING,   AMMUN_BULLET,       qfalse,  qfalse, 18,    qfalse, qtrue,   2500,  0.9f,       0,           0,           qfalse,       qtrue,     qfalse,           qfalse,      qfalse,     qtrue,       qtrue,      qtrue,           qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qtrue,  qtrue,      qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     55,     55,    0,          "MOBILE BROWNING",     WS_BROWNING,        qfalse, qtrue,  qtrue,   qtrue,    450,    1,   150,    0,                  150,                3000,      DELAY_LOW,     66,          0,          20,               1500,   300,     2000,            250,            250,             0,                 1250,               0,        { 12, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "browning",              WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK2, WEAP_ATTACK2,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD3, WEAP_DROP2,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MOBILE_BROWNING,      MOD_MOBILE_BROWNING      },  // WP_MOBILE_BROWNING_SET   // 50
	{ WP_MORTAR2,              ITEM_WEAPON_MORTAR2,               TEAM_AXIS,   SK_HEAVY_WEAPONS,                            WP_MORTAR2_SET,         WP_MORTAR,               WP_NONE,  WP_MORTAR2,           WP_MORTAR2,           AMMUN_BULLET,       qfalse,  qfalse, 1,     qtrue,  qtrue,   0,     0,          0,           0,           qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qtrue,   qfalse,     qfalse, qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "GRANATWERFER",        WS_MORTAR2,         qfalse, qtrue,  qfalse,  qfalse,   15,     1,   1,      15,                 0,                  0,         DELAY_HW,      1600,        0,          0,                0,      0,       0,               250,            250,             0,                 1000,               0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "",                "axis_mortar",           WEAPON_CARD_1,    { 1.f, 1.f, 0.f, 1.f, 0 / 8.f, 1 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHFROM, WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MORTAR2,              MOD_MORTAR2              },  // WP_MORTAR2			     // 51
	{ WP_MORTAR2_SET,          ITEM_WEAPON_MORTAR2_SET,           TEAM_AXIS,   SK_HEAVY_WEAPONS,                            WP_MORTAR2,             WP_MORTAR_SET,           WP_NONE,  WP_MORTAR2,           WP_MORTAR2,           AMMUN_SHELL,        qfalse,  qfalse, 250,   qtrue,  qtrue,   0,     0,          400,         400,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qtrue,      qfalse, qfalse, qtrue,      qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "GRANATWERFER",        WS_MORTAR2,         qfalse, qtrue,  qfalse,  qfalse,   16,     1,   1,      16,                 0,                  0,         DELAY_HW,      1400,        0,          0,                0,      0,       0,               250,            250,             0,                 1667,               0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "mortar_grenade",  "axis_mortar_set",       WEAPON_CARD_NONE, { 1.f, 1.f, 0.f, 1.f, 0.f, 1.f },           WEAP_IDLE2, WEAP_ATTACK1, WEAP_ATTACK1,         WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {.5f, .33f, .33f, .33f, .33f},       MOD_MORTAR2,              MOD_MORTAR2              },  // WP_MORTAR2_SET		     // 52
	{ WP_BAZOOKA,              ITEM_WEAPON_BAZOOKA,               TEAM_ALLIES, SK_HEAVY_WEAPONS,                            WP_NONE,                WP_PANZERFAUST,          WP_NONE,  WP_BAZOOKA,           WP_BAZOOKA,           AMMUN_ROCKET,       qfalse,  qfalse, 400,   qtrue,  qfalse,  0,     0,          400,         300,         qfalse,       qfalse,    qfalse,           qfalse,      qfalse,     qfalse,      qtrue,      qtrue,           qfalse, qtrue,      qfalse,   qfalse,  qfalse,  qtrue,   qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qfalse,       qtrue,        qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qtrue,      0,      0,     0,          "BAZOOKA",             WS_BAZOOKA,         qfalse, qtrue,  qfalse,  qfalse,   4,      1,   1,      4,                  0,                  0,         DELAY_HW,      2000,        0,          0,                0,      0,       0,               250,            250,             250,               250,                32000.f,  { -24, -4, 24 }, { 14, 10, 0 },    1.0f,      0,              0,            0,                 { 0, 0 },       { 0, 0 },      "rocket",          "bazooka",               WEAPON_CARD_3,    { 1.f, 1.f, 0.f, 1.f, 1 / 8.f, 2 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qtrue,        {1, .66f, .66f, .66f, .66f},         MOD_BAZOOKA,              MOD_BAZOOKA              },  // WP_BAZOOKA               // 53
	{ WP_MP34,                 ITEM_WEAPON_MP34,                  TEAM_AXIS,   SK_LIGHT_WEAPONS,                            WP_NONE,                WP_STEN,                 WP_NONE,  WP_MP34,              WP_MP34,              AMMUN_BULLET,       qfalse,  qtrue,  14,    qfalse, qtrue,   200,   0.6f,       0,           0,           qfalse,       qtrue,     qfalse,           qtrue,       qfalse,     qtrue,       qtrue,      qfalse,          qfalse, qfalse,     qfalse,   qfalse,  qfalse,  qfalse,  qfalse,     qfalse, qfalse,          qfalse,  qfalse,     qfalse, qfalse, qfalse,     qtrue,        qfalse,       qfalse,          qfalse,       qfalse,   qfalse,          qfalse,       qfalse,     0,      0,     0,          "MP 34",               WS_MP34,            qtrue,  qtrue,  qtrue,   qtrue,    96,     1,   32,     32,                 32,                 3100,      DELAY_LOW,     150,         0,          15,               1200,   450,     2000,            250,            250,             250,               250,                0,        { 16, -4, 24 },  { 14, 6, -4 },    1.0f,      0.3f,           0.6f,         0,                 { 0, 0 },       { 0, 0 },      "",                "mp34",                  WEAPON_CARD_3,    { 1.f, 1.f, 0.f, 1.f, 2 / 8.f, 3 / 8.f },   WEAP_IDLE1, WEAP_ATTACK1, WEAP_ATTACK_LASTSHOT, WEAP_ALTSWITCHFROM, WEAP_ALTSWITCHTO,   WEAP_RELOAD1, WEAP_RAISE,   WEAP_DROP,    qfalse,       {1, 1, 1, 1, 1},                     MOD_MP34,                 MOD_MP34                 },  // WP_MP34                  // 54   // Solothurn S1-100 SMG
};
...
{
    //ammunition          eType                  eFlags                      svFlags                       content          trType          trTime boundingsBox                                     clipMask          nextThink accuracy effect1Time takedamage health timeStamp                                                                     
    { AMMUN_NONE,         ET_GENERAL,            EF_NONE,                    SVF_NONE,                     CONTENTS_NONE,   TR_STATIONARY,  0,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_ALL,         0,        0,       0,          qfalse,    0,     0,        },
    { AMMUN_MELEE,        ET_GENERAL,            EF_NONE,                    SVF_NONE,                     CONTENTS_NONE,   TR_LINEAR,      0,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_SHOT,        0,        0,       0,          qfalse,    0,     0,        },
    { AMMUN_BULLET,       ET_GENERAL,            EF_NONE,                    SVF_NONE,                     CONTENTS_NONE,   TR_LINEAR,      0,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_SHOT,        0,        0,       0,          qfalse,    0,     0,        },      
    { AMMUN_GRENADE,      ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { -4.f, -4.f, 0.f }, { 4.f, 4.f, 6.f } },      MASK_MISSILESHOT, 2500,     0,       0,          qfalse,    0,     0,        }, 
    { AMMUN_RIFLENADE,    ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { -4.f, -4.f, 0.f }, { 4.f, 4.f, 6.f } },      MASK_MISSILESHOT, 4000,     0,       0,          qfalse,    0,     0,        },
    { AMMUN_ROCKET,       ET_MISSILE,            EF_NONE,                    SVF_BROADCAST,                CONTENTS_NONE,   TR_LINEAR,      -50,   { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_MISSILESHOT, 20000,    4,       0,          qfalse,    0,     0,        },  
    { AMMUN_SHELL,        ET_MISSILE,            EF_NONE,                    SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { -4.f, -4.f, 0.f }, { 4.f, 4.f, 6.f } },      MASK_MISSILESHOT, 0,        0,       0,          qfalse,    0,     0,        },   
    { AMMUN_MAPMORTAR,    ET_MISSILE,            EF_NONE,                    SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_MISSILESHOT, 20000,    4,       0,          qfalse,    0,     0,        },     
    { AMMUN_FLAME,        ET_FLAMETHROWER_CHUNK, EF_NONE,                    SVF_NOCLIENT | SVF_BROADCAST, CONTENTS_CORPSE, TR_DECCELERATE, -50,   { { -4, -4, -4 }, { 4, 4, 4 } },                 MASK_MISSILESHOT, 0,        0,       0,          qfalse,    0,     0,        },
    { AMMUN_ARTY,         ET_MISSILE,            EF_SMOKINGBLACK,            SVF_NOCLIENT,                 CONTENTS_NONE,   TR_STATIONARY,  1,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_MISSILESHOT, 8950,     2,       0,          qfalse,    0,     0,        },     
    { AMMUN_AIRSTRIKE,    ET_MISSILE,            EF_BOUNCE,                  SVF_NONE,                     CONTENTS_NONE,   TR_GRAVITY,     1,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_MISSILESHOT, 1000,     0,       0,          qfalse,    0,     0,        },     
    { AMMUN_CANISTER,     ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { -4.f, -4.f, 0.f }, { 4.f, 4.f, 6.f } },      MASK_MISSILESHOT, 2500,     0,       0,          qfalse,    0,     0,        },  
    { AMMUN_SMOKEGRENADE, ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_NONE,   TR_GRAVITY,     -50,   { { -4.f, -4.f, 0.f }, { 4.f, 4.f, 6.f } },      MASK_MISSILESHOT, 2500,     0,       16,         qfalse,    0,     0,        },
    { AMMUN_DYNAMITE,     ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_CORPSE, TR_GRAVITY,     -50,   { { -12.f, -12.f, 0.f }, { 12.f, 12.f, 20.f } }, MASK_MISSILESHOT, 15000,    0,       0,          qfalse,    5,     16500,    },
    { AMMUN_LANDMINE,     ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_CORPSE, TR_GRAVITY,     -50,   { { -16.f, -16.f, 0.f }, { 16.f, 16.f, 16.f } }, MASK_MISSILESHOT, 15000,    0,       0,          qtrue,     5,     16500,    },
    { AMMUN_SATCHEL,      ET_MISSILE,            EF_BOUNCE_HALF | EF_BOUNCE, SVF_BROADCAST,                CONTENTS_CORPSE, TR_GRAVITY,     -50,   { { -12.f, -12.f, 0.f }, { 12.f, 12.f, 20.f } }, MASK_MISSILESHOT, 0,        0,       0,          qfalse,    5,     0,        },
    { AMMUN_BOX,          ET_GENERAL,            EF_NONE,                    SVF_NONE,                     CONTENTS_NONE,   TR_GRAVITY,     0,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_SHOT,        0,        0,       0,          qfalse,    0,     0,        },     
    { AMMUN_SYRINGUE,     ET_GENERAL,            EF_NONE,                    SVF_NONE,                     CONTENTS_NONE,   TR_LINEAR,      0,     { { 0, 0, 0 }, { 0, 0, 0 } },                    MASK_SHOT,        0,        0,       0,          qfalse,    0,     0,        },     
};
