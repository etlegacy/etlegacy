...
gentity_t *SelectRandomTeamSpawnPoint(int teamstate, team_t team, int majorSpawn, int minorSpawn)
{
	...
	while (count < MAX_TEAM_SPAWN_POINTS && (spot = G_Find(spot, FOFS(classname), classname)) != NULL) {
 ...
 */
static gentity_t *G_FindClosestTeamSpawnEntity(team_t team, vec3_t origin, int minorSpawn) {
	gentity_t *spot = NULL;
	gentity_t *closest = NULL;
	char *classname;
	float shortest = MAX_MAP_SIZE * 2, tmp;
	vec3_t target;
 ...
 * @param[in] team
 * @param[in] resolvedMinorSpawnPt minor spawn pt blob closest to player's selection.
 * @return Spawn point index or -1.
 */
static int G_ResolveSpawnPointIndex(team_t team, gentity_t *resolvedMinorSpawnPt)
{
	int i;
	int closestTeamSpawnPt;
	float closestTeamSpawnPtDist;

	if (resolvedMinorSpawnPt) {
		// find major spawnpt closest to resolvedMinorSpawnPt
		...
		for (i = 0; i < level.numSpawnPoints; i++)
		{
			...
			VectorSubtract(resolvedMinorSpawnPt->s.origin, teamSpawnPointState->origin, diffVector);
...
static int G_ResolveAutoSpawnPointIndex(team_t team, int targetSpawnPt) {
	if (targetSpawnPt >= 0 && targetSpawnPt < level.numSpawnPoints) {
		spawnPointState_t *targetSpawnPointState = &level.spawnPointStates[targetSpawnPt];
...
void G_UpdateSpawnPointStatePlayerCounts()
{
	...
	gentity_t *resolvedMinorSpawnPt = NULL;
	// check updates
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		{
			resolvedMinorSpawnPt = G_FindClosestTeamSpawnEntity(
				client->sess.sessionTeam, level.spawnPointStates[resolvedSpawnPt].origin,
				client->sess.userMinorSpawnPointValue);
			resolvedSpawnPt = G_ResolveSpawnPointIndex(client->sess.sessionTeam, resolvedMinorSpawnPt);
		...
		if (client->sess.userMinorSpawnPointValue <= 0) {
			client->sess.userMinorSpawnPointValue = resolvedMinorSpawnPt ? resolvedMinorSpawnPt->spawnId : -1;
		}
