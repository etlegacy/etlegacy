...
void PersWriteClient(fileHandle_t f, gclient_t *cl)
{
	...
	// save the fields
	for (field = gclientPersFields ; field->len ; field++) // write the block
	{
...
void PersReadClient(fileHandle_t f, gclient_t *cl)
{
	...
	// read the fields
	for (field = gclientPersFields ; field->len ; field++) // read the block
	{
...
void PersWriteEntity(fileHandle_t f, gentity_t *ent)
{
	...
	// save the fields
	for (field = gentityPersFields ; field->len ; field++) // write the block
	{
...
void PersReadEntity(fileHandle_t f, gentity_t *cl)
{
	...
	// read the fields
	for (field = gentityPersFields ; field->len ; field++) // read the block
	{
		trap_FS_Read(( void * )((byte *)cl + field->ofs), field->len, f);
	}
}
