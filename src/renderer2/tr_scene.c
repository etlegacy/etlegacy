...
static void R_AddWorldLightsToScene()
{
	...
	for (i = 0; i < tr.world->numLights; i++)
	{
		...
		if (r_numLights >= MAX_REF_LIGHTS)
		{
			Ren_Print("WARNING R_AddWorldLightsToScene: Dropping light, reached MAX_REF_LIGHTS\n");
			return;
		}
...
void RE_AddDynamicLightToScene(const vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags)
{
	...
	if (flags == REF_DIRECTED_DLIGHT)
	{
		// until implemented, we exit now..
/*
		// or just fake it with a super omni :)   !!!DEBUG!!!  only for test.. remove later.
		// It looks nice on Radar, with the lightning..
		light = &backEndData->lights[r_numLights++];
		//VectorCopy(org, light->l.origin);
		VectorSet(light->l.origin, 0.f, 0.0f, 100000.f); // this is not normalized,.. but it's not straight down for sure (i want to see some shadows)
		// for a directional dlight, origin is a directional normal.  Otherwise it's a position
		VectorCopy(org, light->direction);
		...
		light->l.radius[0] = 100000000;
		light->l.radius[1] = 100000000;
		light->l.radius[2] = 100000000;
...
void RE_RenderScene(const refdef_t *fd)
{
	...
	if (tr.glfogsettings[FOG_WATER].registered && tr.world->globalFog >= 0) //tr.glfogsettings[FOG_MAP].registered)
	{
		...
		{
			//@RE_SetGlobalFog(qfalse, 0, tr.glfogsettings[FOG_WATER].color[0], tr.glfogsettings[FOG_WATER].color[1], tr.glfogsettings[FOG_WATER].color[2], tr.glfogsettings[FOG_WATER].end); // .density);
			//RE_SetGlobalFog(qfalse, 0, tr.glfogsettings[FOG_WATER].color[0], tr.glfogsettings[FOG_WATER].color[1], tr.glfogsettings[FOG_WATER].color[2], tr.glfogsettings[FOG_WATER].color[3]);
			//RE_SetGlobalFog_(qfalse, FOG_WATER);
			//RE_SetFog(FOG_CMD_SWITCHFOG, FOG_WATER, 0, 0, 0, 0, 0);
		...
		{
//RE_SetGlobalFog(qtrue, 0, 0, 0, 0, 0); // restore the fog
//RE_SetGlobalFog_(qtrue, -1);
...
// render the scene for use on the environment-cubemaps.
// This doesn't render fog, lights, shadows, decals, and it's much faster.
// Good enough for a 32x32 reflection cubemap.
// NOTE: Somewhere the RenderScene function messes up fov_x/fov_y of the camera.
//       When using this simpler function, the fov is correct, and the cubemaps have aligned images.
// TODO: check ^^that issue..
void RE_RenderSimpleScene(const refdef_t *fd)
{
	viewParms_t parms;
	int         startTime;
...
//	tr.refdef.numLights = r_numLights - r_firstSceneLight;
	tr.refdef.numLights = 0;
	tr.refdef.lights    = &backEndData->lights[r_firstSceneLight];

	//tr.refdef.num_coronas = r_numcoronas - r_firstSceneCorona;
	tr.refdef.num_coronas = 0;
	tr.refdef.coronas     = &backEndData->coronas[r_firstSceneCorona];
	...
	//tr.refdef.numDecalProjectors = r_numDecalProjectors - r_firstSceneDecalProjector;
	tr.refdef.numDecalProjectors = 0;
	tr.refdef.decalProjectors    = &backEndData->decalProjectors[r_firstSceneDecalProjector];
