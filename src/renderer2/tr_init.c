...
//----(SA)	end
	...
	// Rafael - wolf fog
	r_wolffog = ri.Cvar_Get("r_wolffog", "1", CVAR_ARCHIVE);   // JPW NERVE cheat protected per id request
	// done
...
void R_Init(void)
{
	...
	if (sizeof(glconfig_t) != 7272)
	{
		ri.Error(ERR_FATAL, "Mod ABI incompatible: sizeof(glconfig_t) == %u != 7272", (unsigned int) sizeof(glconfig_t));
	}
...
{
	R_IssuePendingRenderCommands();
	//FIXME there is no method in the import needs to be added
	if (qtrue)   //!ri.Sys_LowPhysicalMemory()
	{
		RB_ShowImages();
	}
}
