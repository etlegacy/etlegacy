...
qboolean Item_SettingShow(itemDef_t *item, qboolean fVoteTest)
{
	...
	if (fVoteTest)
	{
		...
		return((Q_atoi(info) & item->voteFlag) != item->voteFlag);
	}
	...
	{
		return(Q_atoi(info) & item->settingTest);
	}
...
/**
 * @brief Will optionally set focus to this item
 * @param[in] item
 * @param[in] x
 * @param[in] y
 * @return
...
void Item_Text_Paint(itemDef_t *item)
{
	...
	if (item->type == ITEM_TYPE_TIMEOUT_COUNTER && menu != NULL && menu->openTime > 0)
	{
		...
		if (seconds <= 2)
		{
			...
			Com_sprintf(text, sizeof(text), textPtr, va("^1%d^*", seconds));
		}
		else
		{
			//Com_sprintf( text, 255, "%d", seconds );
			Com_sprintf(text, sizeof(text), textPtr, va("%d", seconds));
		}
